...
namespace ApiCognosV1.Controllers
{
    ...
    public class scidController : ControllerBase
    {
        ...
        {
            return _context.Maestro_pruebas.Where(e => e.maestro_id_paciente == Id && e.maestro_tipo_prueba == 5).OrderByDescending(e => e.maestro_fecha).ToList();
        }

        [HttpGet]
        [Route("MaestroHistSCIDList/{Id}")]
        public IEnumerable<Maestro_pruebas_hist> MaestroHistSCIDList(int Id)
        {
            return _context.Maestro_pruebas_hist.Where(e => e.maestro_id_paciente == Id && e.maestro_tipo_prueba == 5).OrderByDescending(e => e.maestro_fecha).ToList();
        }
        ...
        [HttpPost]
        [Route("MaestroHistSCID")]
        public IActionResult InsertMaestroHistSCID(int maestro_id_paciente, string fecha,string observ)
        {
        ...
        //Cambios en modulo de resultados
        //Se eliminan los id y se vuelven ainsertar para mostrarse en el informe
        [HttpDelete]
        [Route("AsignarPruebasScid")]
        public IActionResult DeleteMostrarSCID(string ids, int prueba, int exp, string imagen_id)
        {
            var respuesta = new Respuesta();
        ...
        [HttpPost]
        [Route("GuardarImagenScidHist")]
        public IActionResult GuardarImagenScidHist(IFormFile files, int id_pac, int tipo_prueba, int maestro_id)
        {
            var respuesta = new Respuesta();
        ...
        [HttpGet]
        [Route("ArchivosSCIDInforme/{IdPac}")]
        public IActionResult ImagenesSCID(int IdPac)
        {
            List<mostrar_exp> MostrarExp = _context.mostrar_exp.Where(x => x.most_tipo_prueba == 5 && x.most_expediente == IdPac).ToList();
